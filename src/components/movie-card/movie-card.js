...
export default class MovieCard extends React.Component {
  handleRatingChange = (rating) => {
    ...
    fetch(
      `https://api.themoviedb.org/3/movie/${id}/rating?api_key=87816a77eb101eb61635d1fc67cd33f4&guest_session_id=${guest_session_id}`,
      {
        method: 'POST',
        headers: {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error('Произошла ошибка при отправке рейтинга')
        }
      })
      .catch((error) => {
        console.error('Произошла ошибка при отправке рейтинга:', error)
      })
  }
